...
-- Per MQ's own docs: always resolve via MacroQuest.Path, never hardcode —
-- these locations are configurable and may not be in their default spots.
local function prozpLogDir()
    if not _dirCache then
        _dirCache = mq.TLO.MacroQuest.Path('logs')() .. '/ProZP'
...
-- The character this log file/console actually belongs to — distinct from any
-- group/tank label a caller embeds in the message text (e.g. Roster.MyGroupId()),
-- which identifies "whose group" rather than "who wrote this line".
local function myName()
    if not _meCache then
        _meCache = mq.TLO.Me.CleanName()
    end
    return _meCache
end
...
local function getConsole()
    if not _console then
        _console                = ImGui.ConsoleWidget.new('##prozp_console')
...
local function ensureFile()
    ...
    local path = string.format('%s/ConsoleLogs_%s_%s.log',
        prozpLogDir(), serverTag(), mq.TLO.Me.CleanName())
    _fh = io.open(path, 'a')
    if _fh then
...
local function log(levelNum, fmt, ...)
    ...
    getConsole():AppendText(string.format('%s[%s][%s] %s', ts, tag, myName(), msg))

    if _config:Get('LogToFile') then
        ensureFile()
        if _fh then
            _fh:write(string.format('[%s][%s][%s] %s\n', os.date('%H:%M:%S'), tag, myName(), msg))
            _fh:flush()
        end
    else
        closeFile()
